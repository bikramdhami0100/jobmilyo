  ...
  try {
    ...
    if (!token) {
      if (!user.userVerify) {
        ...
        const newToken = jwt.sign({ encodeemail: email, role: 'user' }, process.env.ADMINTOKEN_SECRETKEY, { expiresIn: '7d' });
      ...
      if (isPasswordValid) {
        ...
        const newToken = jwt.sign({ encodeemail:user, role: 'user' }, process.env.ADMINTOKEN_SECRETKEY, { expiresIn: '7d' });
