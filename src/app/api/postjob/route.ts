...
export async function GET(req: any) {
  ...
  try {
    ...
    const jobs = await UserPostedJob.find().sort({ jobupload: -1 }).limit(5);

    return NextResponse.json({ message: "Successfully fetched jobs", status: 200, data: jobs,user:userdetail });
  } catch (error:any) {
